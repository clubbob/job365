...
export default function LegalPageShell({ title, effectiveDate, children }: LegalPageShellProps) {
  return (
    <div className="flex flex-col gap-5">
      <header className="rounded-xl border border-border bg-surface px-6 py-6 shadow-card sm:px-8">
      ...
      <article className="rounded-xl border border-border bg-surface p-6 shadow-card sm:p-8">
        <div className="space-y-8 [&_strong]:font-semibold [&_strong]:text-foreground">{children}</div>
      </article>
